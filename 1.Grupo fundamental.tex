...
donde $\overline{\alpha}(s)=\alpha(1-s)$. Y tenemos el lazo constante $c_x:I\to X$ tal que $c_{x_0}(s)=x_0$, para toda $s\in I$.
    ...
    \end{definicion}
    \begin{nota}
        Cuando $f,g:X\to Y$ tengan algunas \textit{propiedades extras} siempre vamos a pedir que una homotopía $H$ entre $f$ y $g$ cumpla que, para cada $t\in I$, $H_t:X\to Y$ tenga esa propiedad también. Por ejemplo, si $f,g$ son funciones basadas, entonces $H_t$ debe ser una función basada.
    \end{nota}
    \begin{ejercicio}
    % ID: 3
    ...
    \end{definicion}
    La definición anterior también se puede dar en los espacios basados, pidiendo funciones continuas basadas $f:(X,x_0)\to (Y,y_0)$ y $g:(Y,y_0)\to (X,x_0)$ tal que $g\circ f\simeq \id_X$ y $f\circ g\simeq \id_Y$ \textit{homotópicos como funciones basados}. En este caso, escribimos $(X,x_0)\simeq (Y,y_0).$
    \begin{ejercicio}
    % ID: 4
...
\end{tikzcd}
    ...
    \begin{center}
        \begin{tikzcd}[row sep=1.25cm, column sep=large]
    ...
    \end{ejercicio}
    \begin{nota}
        Si hay puntos $x_0,x_1\in X$ tal que $f(x_0)=g(x_0)$ y $f(x_1)=g(x_1)$, entonces la homotopía $H$ de la proposición \ref{prop:homotopia convexa} es relativa a $\{x_0,x_1\}$ ya que 
        \begin{align*}
    ...
    \end{nota}
    \begin{lema}
        Sea $X$ un espacio topológico, $\alpha,\alpha':I\to X$ trayectorias de $x_0$ a $x_1$, $\beta,\beta':I\to X$ trayectorias de $x_1$ a $x_2$ tal que $\alpha\simeq\alpha'$ rel $\{0,1\}$ y $\beta\simeq\beta'$ rel $\{0,1\}$. Entonces $\alpha*\beta\simeq \alpha'*\beta'$ rel $\{0,1\}.$
    \end{lema}
    \begin{dem}
        Tenemos 
        \begin{align*}
    ...
    \begin{align*}
        ...
        &H(1,t)=G(1,t)=x_2,
    \end{align*}
    y, como $H$ es continua, por el lema del pegado, $H$ es testigo de que $\alpha*\beta\simeq \alpha'*\beta'$ rel $\{0,1\}$.
    \end{dem}
    \begin{coro}
